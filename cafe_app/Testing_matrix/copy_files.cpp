...
void copy_directory(const fs::path& source, const fs::path& destination) {
  ...
  for (const auto& entry : fs::recursive_directory_iterator(source)) {
    ...
    if (fs::is_directory(path_in_source)) {
      ...
      fs::copy_file(path_in_source, path_in_destination,
...
int main(int argc, char* argv[]) {
  try {
    // Adjust paths to work relative to the current directory of
    // "Matrice_testare"
    fs::path source = argv[1];
    fs::path destination = argv[2];

    std::cout << "Copying data from '" << source << "' to '" << destination
              << "'..." << std::endl;
