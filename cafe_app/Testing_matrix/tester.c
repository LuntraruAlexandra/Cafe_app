...
int main() {
  ...
  for (int i = 1; i <= 20; i++) {
    ...
    snprintf(output_file, sizeof(output_file), "%s/output%d.txt", output_dir,
             i);

    // Step 1: Run copy_files
    run_command(copy_files,
                (char *[]){copy_files, changed_data, initial_data, NULL});
    ...
    if (pid == 0) {
      ...
      waitpid(pid, &status, 0);
      if (WIFEXITED(status) && WEXITSTATUS(status) != 0) {
        fprintf(stderr, "app failed with exit code %d\n", WEXITSTATUS(status));
        exit(1);
      }
    }
    ...
    run_command(compare_files,
                (char *[]){compare_files, initial_data, changed_data, NULL});
  }
