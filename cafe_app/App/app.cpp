...
// Singleton Design Pattern because we only need one instance to create the
// folders
class CreateFolders {  // class that creates the 5 folders that correspond to
                       // every city
 private:
 ...
 public:
  ...
  void createDirectories() {
    vector<string> cities = {"București", "Cluj-Napoca", "Timișoara", "Iași",
                             "Brașov"};
    ...
    for (const auto& city : cities) {
      try {
        ...
        cerr << "Error creating directory (" << city << "): " << e.what()
             << endl;
      }
    }
  }
  void createCityFiles() {
    ...
    tm* now = localtime(&t);
    string header_report = to_string(now->tm_mday) + "/" +
                           to_string(now->tm_mon + 1) + "/" +
                           to_string(now->tm_year + 1900) + "\n";
    vector<string> cities = {"București", "Cluj-Napoca", "Timișoara", "Iași",
                             "Brașov"};
    vector<string> files = {
        "employees.csv", "products.csv", "orders.csv", "recipes.csv",
        "report.csv",    "events.csv"};  // i will use recipes.csv for the
                                         // products that are not premade.
    // it will also interact with products.csv which contains the ingredients
    vector<string> headers = {
        "Name,Role,Starting hour,Ending hour,Salary\n",
        "Product name,Quantity,Price,Type\n",  // type will be used to know if
                                               // it is ingredient or part of
                                               // menu
    ...
    for (const auto& city : cities) {
      ...
      for (const auto& file : files) {
        ...
        try {
          ...
          cerr << "Error creating file (" << filePath << "): " << e.what()
               << endl;
        }
        header_index++;
      }
...
CreateFolders* CreateFolders::getInstance() {
  call_once(flag, []() { instance = new CreateFolders(); });
  return instance;
}
// this function will be used by multiple classes in order to interact with
// products.csv easily
pair<float, float> changeQuantityInProducts(
    string city, string request,
    float change) {  // change represents a change in quantity.change can be
                     // negative or positive, depending on the action
  float final_price = -1;
  float rest_quantity;
  string filePath1 = in_folder + "/" + city + "/" + "products.csv";
  // firstly we need to check if we can find the order in products.csv if it's a
  // premade good
  ...
  while (getline(productsFilein, line)) {
    ...
    if (product != request) {
      ...
      line = product + "," + new_quantity + "[" + unit + "]" + "," + price +
             "," + type + ",";
      lines.emplace_back(line);
    }
  }
...
bool isToday(const string& dateStr) {
  ...
  // format "dd/mm/yyyy"
  ss >> eventDate.tm_mday >> delimiter >> eventDate.tm_mon >> delimiter >>
      eventDate.tm_year;
    ...
    try {
      ...
      if (!isValidDate(date)) {
        throw invalid_argument(
            "Invalid date format or out-of-range values. Please try again.");
      }
      break;
    } catch (const exception& e) {
...
void changeReport(string city, float price, string type) {
  ifstream reportIn(
      in_folder + "/" + city + "/" +
      "report.csv");  // add the new expenses for refilling the supply
  string line_report_date, line_report_profit, line_report_expenses;
  getline(reportIn, line_report_date);  // first one is current date, second one
                                        // profit, last one expense
...
class Handler {
 ...
 public:
  ...
  void setNextHandler(shared_ptr<Handler> handler) { nextHandler = handler; }

  virtual void handleRequest(const string& customer_name,
 ...
 public:
  ...
  void checkMenu() {
    ...
    while (getline(fin_recipes, line)) {
      ...
      color++;
      if (lang == "ro")
        cout << "* " << translate_to_romanian(recipe) << "   " << price << "  *"
             << RESET << endl;
      else
    ...
    while (getline(fin_products, line)) {
      ...
      if (type == "1") {
        ...
        if (lang == "en")
          ...
          cout << "* " << translate_to_romanian(product) << "   " << price
               << "  *" << RESET << endl;
      }
    }
    fin_recipes.close();
  ...
  void handleRequest(const string& customer_name,
                     const vector<string>& requests) override {
...
class WaiterHandler : public Handler {
 ...
 public:
  ...
  void handleRequest(const string& customer_name,
    ...
    // if a waiter took an order he will be popped out of the queue and put in
    // the back so the next one can come for the next order i als took into
    // consideration to put in the queue only tha waiters available at the
    // current hour
    if (waiterQueue.empty()) {
      if (lang == "en")
        cout << "Sorry, but there are no available waiters at this time!Please "
                "try again later!\n";
      else
        cout << "Ne pare rau dar nu sunt chelneri disponibili în acest monet! "
                "Te rugăm, încearcă mai târziu!\n";
      return;
    }
 ...
 public:
  ...
  void handleRequest(const string& customer_name,
    ...
    if (baristaQueue.empty()) {
      if (lang == "en")
        cout << "Sorry, but there are no available baristas at this "
                "time!Please try again later!\n";
      else
        cout << "Ne pare rău, dar nu este nicio barista disponibilă în acest "
                "moment!Te rugăm, încearca mai târziu!\n";
      return;
    }
    ...
    for (const auto& request : requests) {
      ...
      pair<float, float> res =
          changeQuantityInProducts(city, translate_to_english(request), -1);
      ...
      if (final_price == -2)
        ...
        if (lang == "en")
          cout << "Hi, " << customer_name << "!Here is your: " << request
               << "⸜(｡˃ ᵕ ˂ )⸝♡\n";
        else
          cout << "Hei, " << customer_name << "!Comanda ta: " << request
               << "⸜(｡˃ ᵕ ˂ )⸝♡\n";
        // total_sum=total_sum + final_price;
      } else {
        final_price = 0;
        // then maybe your desired product has to be prepared
        // I will parse the recipe.csv in order to get the list of ingredients
        // and their quantities
        ofstream tempout(in_folder + "/" + city + "/" + "temp.txt");
        bool found_recipe;
        ifstream fiin(filePath1);
        // copy the content of products.csv in order to restore it if there are
        ...
        while (getline(fin, line)) {
          ...
          if (recipe == translate_to_english(request)) {
            ...
            while (ingQuan != "") {
              ...
              pair<float, float> res =
                  changeQuantityInProducts(city, ingredient, -q);
              // cout<<res.second<<endl;
              getline(ss, ingQuan, ',');
              if (res.second <
                  0) {  // can't have negative quantities so the recipe is not
                        // possible. Then products.csv has to be restored to
                        // original state
        ...
        if (enough_ingredients == false) {
          if (lang == "en")
            cout << "Sorry but the recipe is not available now! Please try "
                    "another one or wait a moment...\n";
          else
            cout << "Ne pare rău, dar rețeta nu este disponibilă! Te rugăm să "
                    "încerci alta sau să așteptți un moment...\n";
          available = false;
          ofstream o(filePath1);
          // if there are not enough ingredients restore the file to its
          // original state
          ...
          if (lang == "en")
            ...
            cout << "Hei, " << customer_name
                 << "!Aici este comanda ta: " << request << "(∗´ര ᎑ ര`∗)\n";
        }
        ...
        if (customer == customer_name) {
          ...
          if (available) {
            cout << "Price:" << final_price - final_price * discount_int / 100
                 << "\n~~~~~~~~~~~~~~~~~\n";  // apply the discount obtained
                                              // from the prevoius order
            total_sum =
                total_sum + (final_price - final_price * discount_int / 100);
          }
          if (ord >= 10 && suma >= 200) {
            discount_int = 8;
          } else if (ord >= 10 && ord < 20)
            discount_int = 5;
          else if (suma >= 200)
            discount_int = 5;
          else if (ord >= 20 && ord < 30)
            discount_int = 4;
          else if (ord >= 30)
            discount_int = 7;

          discount_int = 0;  // we need to make sure that the same discount is
                             // applied only once
      ...
      finOrders.close();
      if (ok ==
          true) {  // then rewrite the entire file using the vector of lines
        ...
        // then just print the initial price and append the new customer in
        // orders.csv
...
// facade design pattern that helps the manager to handle product supply and
// employees file subsystem 1
class Employees {
 public:
  ...
  void addEmployee(string city, string name, string role, int starting_hour,
                   int ending_hour, float salary) {
    string filePath = in_folder + "/" + city + "/" + "employees.csv";
    ofstream out(filePath, ios::app);  // append the new line with ios::app
    out << name << "," << role << "," << starting_hour << "," << ending_hour
        << "," << salary << ",\n";
    out.close();
  }
...
class Products {
 public:
  ...
  void addProduct(string city, string product, string quantity, float price,
                  int type) {
    string filePath = in_folder + "/" + city + "/" + "products.csv";
    ofstream out(filePath, ios::app);  // append the new line with ios::app
...
class Event {
 ...
 public:
  Event() {
    ...
    date = "28/01/2024";
    special_products = {"icy donut;100[pcs];15", "Snow white cake;40[pcs];16",
                        "Oriental juice;2[l];13"};
    description = "Cafe's anniversary";
    cost = 2000;
  }

  Event(string _event, string _date, vector<string> _special_products,
        string _description, int _cost, string city) {
    event = _event;
    date = _date;
  ...
  void addSpecialProducts() {
    ...
    // they will be recognized by their type which is the date of the event and
    // only that particular date would enable them
...
class Manager {
 ...
 public:
  string city;
  // default constructor
  Manager() { city = "București"; }
  // constructor for the attribute city
  Manager(string City) { city = City; }
  // the menu for manager will have 1.Handle employees/n2.Handle
  // products/ng3.New Recipe
      ...
      try {
        ...
        if (option == 1)
          ...
          regex quantityPattern(
              R"(^\d+(\.\d+)?\[(pcs|l|kg)\]$)");  // regex for validating if
                                                  // data has the right pattern
                                                  // quantity[unit]

          while (true) {
            try {
              cout << "Quantity: Please enter a valid format! (ex: 20[pcs], "
                      "10[l], 15[kg])\n";
              cin >> quantity;

              if (!regex_match(quantity, quantityPattern)) {
                throw invalid_argument(
                    "Invalid format. Please use the format like 20[pcs], "
                    "10[l], or 15[kg].");
              }
              // if the pattern is valid we can continue reading data so we exit
              // the loop
              break;
            } catch (const exception& e) {
              cout << "Error: " << e.what() << endl;
          ...
          cin >> price;
          int type;
          cout << "Type:(0 for ingredient, 1 for product that will appear on "
                  "the MENU):\n";  // there woul pe special type as a date in
                                   // format DD/MM/YYYY  for events
          cin >> type;
          ...
          while (true) {
            try {
              cout << "How much would you like to add? Quantity: Please enter "
                      "a valid format! (ex: 20[pcs], 10[l], 15[kg])\n";
              cin >> quantity;
          ...
          pair<float, float> res =
              changeQuantityInProducts(city, product, stof(quantity));
  ...
  void newRecipes() {
    ...
    for (int i = 0; i < number_of_ingredients; i++) {
      ...
      string quantity;
      regex quantityPattern(
          R"(^\d+(\.\d+)?\[(pcs|l|kg)\]$)");  // regex for validating if data
                                              // has the right pattern
                                              // quantity[unit]

      while (true) {
        try {
          cout << "Quantity: Please enter a valid format! (ex: 20[pcs], 10[l], "
                  "15[kg])\n";
          cin >> quantity;
          cin.ignore();
          ...
          if (!regex_match(quantity, quantityPattern)) {
            throw invalid_argument(
                "Invalid format. Please use the format like 20[pcs], 10[l], or "
                "15[kg].");
          }
          // if the pattern is valid we can continue reading data so we exit the
          // loop
          break;
        } catch (const exception& e) {
          cout << "Error: " << e.what() << endl;
  ...
  void HandleEvent() {  // this function may be used for future events but it is
                        // also used to update todays event
    string event;
    vector<string> special_products;
    ...
    cout << "Enter special products in the format: "
            "name;quantity[unit];price/unit (type 'done' to finish):"
         << endl;
    while (true) {
      try {
        cout << "Product name:\n";
        ...
        if (!regex_match(product, productPattern)) {
          throw invalid_argument(
              "Invalid format! Use the format: name;quantity[unit];price/unit, "
              "where unit is one of [pcs], [l], or [kg].");
        }
...
class EventProduct {
 ...
 public:
  ...
  void productDisplay() {
    if (lang == "ro")
      cout << translate_to_romanian(name) << ": " << quantity << unit
           << " preț: " << price << endl;
    else
...
class TodaysEvent {
 private:
  ...
  TodaysEvent(const string& _city) : city(_city) {
    ...
    while (getline(file, line)) {
      ...
      if (isEventToday()) {
        ...
        while (getline(productsStream, productData, ')')) {
          ...
          // cout<<price<<endl;
          products.push_back(
              EventProduct(productName.substr(1), quantity, unitStr, price));
        }
        // cout<<costStr;
        cost = stod(costStr);
    ...
    if (!eventFound) {
      ...
      changeReport(city, cost, "expense");
  }
  // ~TodaysEvent() {
 ...
 public:
  ...
  void updateEvent(const string& _event_name, const string& _description,
                   vector<EventProduct> _products, double _cost) {
  ...
  void order(string name, vector<string> requests) {
    ...
    for (const string& request : requests) {
      ...
      for (auto& product : products) {
        if (product.name == request ||
            product.name == translate_to_english(request)) {
    ...
    while (getline(finOrders, line)) {
      ...
      if (customer == name) {
        ...
        line = customer + "," + to_string(ord) + "," + to_string(suma) + "," +
               to_string(discountInt) + ",";
      }
    ...
    if (!customerFound) {
      ...
      if (lang == "en")
        ...
        cout << "Hei, " << name
             << "! Comanda ta din cadrul evenimentului este de: " << price
             << "\n";
    }
    ...
    if (!inputFile.is_open() || !tempFile.is_open()) {
      cerr << "Error opening file for reading or writing: " << filePath
           << " or " << tempFilePath << endl;
      return;
    }
      ...
      if (isEventToday()) {
        ...
        for (const auto& product : products) {
          updatedProductsStr += "(" + product.name + ";" +
                                to_string(static_cast<int>(product.quantity)) +
                                "[" + product.unit + "];" +
                                to_string(static_cast<int>(product.price)) +
                                ")";
        }
        tempFile << event << "," << date << "," << updatedProductsStr << ","
                 << description << "," << costStr << endl;
      } else {
        tempFile << line << endl;
      }
    }
    ...
    if (remove(filePath.c_str()) != 0 ||
        rename(tempFilePath.c_str(), filePath.c_str()) != 0) {
  ...
  void printEventDetails() const {
    if (lang == "ro") {
      ...
      cout << "Produse: " << endl;
      for (const auto& product : products) {
        cout << "- " << translate_to_romanian(product.name) << ": "
             << product.quantity << " " << product.unit
             << ", preț: " << product.price << endl;
      }
    } else {
      ...
      for (const auto& product : products) {
        cout << "- " << product.name << ": " << product.quantity << " "
             << product.unit << ", price: " << product.price << endl;
      }
    }
  }
};
...
// Observer Pattern for manager to get notifications about employees and
// products
template <typename T>
class Notifier {
 public:
  void notify(const T& message) { notifications.push_back(message); }
 ...
 private:
  ...
  void displayMessage(const T& message) {
    // Default behavior pentru tipurile generice (poate fi personalizat pentru
    // alte tipuri)
    cout << "- " << message << endl;
  }
};
class EmployeesNotifier {
 ...
 public:
  EmployeesNotifier(const string& city, Notifier<string>* notifier)
  ...
  void notifyGaps(const string& role, const vector<bool>& coverage) {
    ...
    for (int i = 0; i < 24; i++) {
      if (!coverage[i] && start == -1) {
        ...
        notifier->notify(role + " gap: " + to_string(start) + ":00 - " +
                         to_string(i) + ":00");
        start = -1;
      }
    }
  ...
  void checkProductQuantities() {
    ...
    while (getline(file, line)) {
      ...
      try {
        ...
        if (quantity <= 1) {
          notifier->notify("Low stock for product: " + productName + " (" +
                           quantityStr + ") in " + city);
        }
      } catch (const exception& e) {
        notifier->notify("Error processing product: " + productName + ". " +
                         e.what());
      }
    }
...
// command pattern for processing different cases of user's interactions with
// the system
// the difference between command pattern and facade is that command pattern
// converts requests into objects we may need different types of objects, for
// manager, customer... this class will constanly interact with the class
// TodayEvent
class UserInterface {
 public:
  void CustomerMenu(shared_ptr<CustomerHandler> customer,
    ...
    while (true) {
      try {
        ...
        cout << (lang == "en" ? "Enter your choice: "
        ...
        if (choice == 1) {
          ...
          cout << (lang == "en" ? "Enter your order (type 'done' to finish):\n"
                                : "Introduceti comanda (scrieti 'done' pentru "
                                  "a termina):\n");
          string item;
          while (true) {
          ...
          cin.ignore();
          getline(cin, name);
          cout << (lang == "en" ? "Enter special products from event: when "
                                  "finished type 'done'\n"
                                : "Introduceti produse speciale din eveniment: "
                                  "cand terminati, scrieti 'done'\n");
          ...
          throw invalid_argument(
              lang == "en" ? "Invalid choice. Please enter a valid option."
                           : "Optiune invalida. Va rugam sa introduceti o "
                             "optiune valida.");
        }
      } catch (const exception& e) {
        cout << "Error: " << e.what() << endl;
    ...
    while (true) {
      try {
        ...
        switch (choice) {
          ...
          default:
            throw invalid_argument(
                "Invalid choice. Please select a valid number between 1 and "
                "5.");
        }
      } catch (const exception& e) {
        cout << "Error: " << e.what() << endl;
